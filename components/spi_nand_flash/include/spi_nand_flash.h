...
struct spi_nand_flash_config_t {
    ...
    /* AirGradient: software chip-select hook.  When set, the device must be
     * registered with spics_io_num = -1; every transaction is then bracketed by
     * spi_device_acquire_bus() + cs_hook(ctx, true) ... cs_hook(ctx, false) +
     * spi_device_release_bus().  The hook runs in the calling task context, so
     * it may block (I2C expander on GO v2.0). */
    void (*cs_hook)(void *ctx, bool assert_cs);
    void *cs_hook_ctx;
};
