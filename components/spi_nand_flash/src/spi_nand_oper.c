...
esp_err_t spi_nand_execute_transaction(spi_nand_flash_device_t *handle, spi_nand_transaction_t *transaction)
{
    ...
    esp_err_t ret;
    if (handle->config.cs_hook != NULL) {
        /* AirGradient: hold the bus so no other device can clock the NAND while
         * its chip-select is asserted by the (slow, task-context) hook. */
        ret = spi_device_acquire_bus(handle->config.device_handle, portMAX_DELAY);
        if (ret != ESP_OK) {
            return ret;
        }
        handle->config.cs_hook(handle->config.cs_hook_ctx, true);
        ret = spi_device_transmit(handle->config.device_handle, (spi_transaction_t *) &e);
        handle->config.cs_hook(handle->config.cs_hook_ctx, false);
        spi_device_release_bus(handle->config.device_handle);
    } else {
        ret = spi_device_transmit(handle->config.device_handle, (spi_transaction_t *) &e);
    }
    if (ret == ESP_OK) {
